...
async function requestMpcSign(hash: string, accountKey: string): Promise<string> {
  ...
  return createResponse.txKey;
}

async function retrieveSig(txKey: string): Promise<string> {
  // wait and get sig from "v1/transactions/mpcsign/one" api
  const retrieveRequest: RetrieveMpcSignRequest = {
    txKey
  };

  for (let i = 0; i < 100 ; i++) { 
    const retrieveResponse = await client.doRequest<RetrieveMpcSignRequest, RetrieveMpcSignResponse>('/v1/transactions/mpcsign/one', retrieveRequest);
    ...
    if (retrieveResponse.transactionStatus === 'COMPLETED' && retrieveResponse.transactionSubStatus === 'CONFIRMED'){
      return retrieveResponse.hashs[0].sig;
    }
...
async function main(){
  ...
  const mpcSignTxKey = await requestMpcSign(hash, getConfigValue('ACCOUNT_KEY'));
  console.log(`transaction created, txKey: ${mpcSignTxKey}`);
  // Get sig
  const mpcSig = await retrieveSig(mpcSignTxKey);
