...
import {CreateWeb3EthSignTransactionRequest, OneWeb3SignRequest, Web3Api} from "../../src/safeheron/web3Api";
...
async function main() {
    ...
    const request: CreateWeb3EthSignTransactionRequest = {
        ...
        transaction: {
            ...
            maxPriorityFeePerGas: feeEstimate.maxPriorityFeePerGas.toString(),
            maxFeePerGas: feeEstimate.maxFeePerGas.toString(),
            nonce: nonce,
            data: data
        }
    };

    const txResult = await web3Api.createWeb3EthSignTransaction(request);
...
async function retrieveSig(web3Api: Web3Api, txKey: string): Promise<string> {
    ...
    for (let i = 0; i < 100; i++) {
        ...
        if (retrieveResponse.transactionStatus === 'SIGN_COMPLETED') {
            return retrieveResponse.transaction.sig.sig
        }
